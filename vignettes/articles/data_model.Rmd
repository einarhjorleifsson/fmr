...
Each survey is identified by a unique value stored as *survey_id*. Intuitively one expects that each survey_id constitutes a unique number for each landing site (*site*) by each sampling day (*date*). This is however not necessarily the case as demonstrated here:
...
Of course on can access the wide tables via {fmr} as shown below, but it is not recommended to use them for downstream analysis in R:
